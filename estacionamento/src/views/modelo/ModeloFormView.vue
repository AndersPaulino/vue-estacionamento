<template>
    ...
    <div class="container" style="background: ;">
      ...
      <div class="row">
        <div class="col-md-12 text-start">
          ...
          <select :disabled="form === 'excluir' ? '' : disabled" class="form-control" v-model="modelo.marca">
          <option v-for="marca in marcas" :key="marca.id" :value="condutor.id">{{ marca.marca }}</option>
        </select>
        </div>
      </div>
  ...
  export default defineComponent({
    ...
    methods: {
      ...
      onClickCadastrar() {
        ModeloClient.cadastrar(this.modelo, this.modelo.marca)
          .then(sucesso => {
